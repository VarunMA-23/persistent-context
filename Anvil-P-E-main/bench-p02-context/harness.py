...
def _run_one_seed(
    ...
    try:
        ...
        # We ingest eval events incrementally to prevent lookahead leakage.
        # Everything from the end of training up to the first signal's window.
        eval_ptr = 0
        
        def ingest_up_to(target_ts: str):
            nonlocal eval_ptr
            to_ingest = []
            while eval_ptr < len(ds.eval_events) and ds.eval_events[eval_ptr]["ts"] <= target_ts:
                to_ingest.append(ds.eval_events[eval_ptr])
                eval_ptr += 1
            if to_ingest:
                adapter.ingest(to_ingest)
